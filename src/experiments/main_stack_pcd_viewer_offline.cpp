...
// void read_graph_file(std::string &graph_file_path, std::vector<int>
// &frame_inds,
...
int main(int argc, char **argv) {
  ...
  int start_ind = std::stoi(std::string(argv[4]));
  int stop_ind = std::stoi(std::string(argv[5]));
  // int total_graphs = std::stoi(argv[2]);
  // int is_auto_preceed = std::stoi(std::string(argv[4]));

  cvo::DatasetHandler *datasetHandler;

  if (dataset_type == "tartanair") {
    datasetHandler = new cvo::TartanAirHandler(dataset_folder);
    auto tartanair = dynamic_cast<cvo::TartanAirHandler *>(datasetHandler);
    tartanair->set_depth_folder_name("deep_depth_nosky");
  } else if (dataset_type == "tum") {
    datasetHandler = new cvo::TumHandler(dataset_folder);
  }
  if (stop_ind < 1) {
    stop_ind = datasetHandler->get_total_number() - 1;
  }
  std::cout << "Plot range: (" << start_ind << ", " << stop_ind << ")"
            << std::endl;
  ...
  std::vector<cvo::Mat34d_row, Eigen::aligned_allocator<cvo::Mat34d_row>>
      poses_all;
  poses_all.resize(datasetHandler->get_total_number());
  read_pose_file(traj_path, poses_all);
  ...
  for (int f = start_ind; f < stop_ind; f++) {
    viewer->addOrUpdateText(std::to_string(f), 0, 0, "title");

    datasetHandler->set_start_index(f);

    cv::Mat source_rgb, source_dep;
    datasetHandler->read_next_rgbd(source_rgb, source_dep);
    ...
    std::shared_ptr<cvo::ImageRGBD<uint16_t>> source_raw(
        new cvo::ImageRGBD<uint16_t>(source_rgb, source_dep_data));
    ...
    // std::string frame_fname(dataset_folder + "/" +
    //                         std::to_string(curr_frame_id) + ".pcd");
    // pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud(
    //     new pcl::PointCloud<pcl::PointXYZRGB>);
    // pcl::io::loadPCDFile<pcl::PointXYZRGB>(frame_fname, *cloud);

    pcl::PointCloud<pcl::PointXYZRGB> cloud;
    source->export_to_pcd(cloud);

    for (auto &&p : cloud) {
      Mat34d_row transform = Eigen::Map<Mat34d_row>(poses_all[f].data());
    ...
    // of points\n"; pcs.push_back(pc);
    viewer->addColorPointCloud(cloud, std::to_string(f));

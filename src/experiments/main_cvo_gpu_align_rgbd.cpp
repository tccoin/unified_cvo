...
int main(int argc, char *argv[]) {
  ...
  std::cout << "write ell! ell init is " << cvo_align.get_params().ell_init
            << std::endl;

  // cvo::cvo
  // cvo_align_cpu("/home/rayzhang/outdoor_cvo/cvo_params/cvo_params.txt");
  Eigen::Matrix4f init_guess =
      Eigen::Matrix4f::Identity();  // from source frame to the target frame
  init_guess(2, 3) = 0;
  ...
  std::vector<uint16_t> source_dep_data(source_dep.begin<uint16_t>(),
                                        source_dep.end<uint16_t>());

  // std::shared_ptr<cvo::Frame> source(new cvo::Frame(start_frame, source_rgb,
  // source_dep, 19, semantics_source,
  //                                                  calib, 1));
  std::shared_ptr<cvo::ImageRGBD<uint16_t>> source_raw(
      new cvo::ImageRGBD(source_rgb, source_dep_data));
  std::shared_ptr<cvo::CvoPointCloud> source(new cvo::CvoPointCloud(
      *source_raw, calib
  ...
  for (int i = start_frame; i < min(total_iters, start_frame + max_num) - 1;
       i++) {
    // calculate initial guess
    std::cout << "\n\n\n\n============================================="
              << std::endl;
    std::cout << "Aligning " << i << " and " << i + 1 << " with GPU "
              << std::endl;
    ...
    std::vector<uint16_t> target_dep_data(dep.begin<uint16_t>(),
                                          dep.end<uint16_t>());

    // std::shared_ptr<cvo::Frame> target(new cvo::Frame(i+1, rgb, dep,
    // calib,1));
    std::shared_ptr<cvo::ImageRGBD<uint16_t>> target_raw(
        new cvo::ImageRGBD(rgb, target_dep_data));
    ...
    double in_product =
        cvo_align.inner_product_cpu(*source, *target, result, ell_init);
    // double in_product_normalized = cvo_align.inner_product_normalized();
    // int non_zeros_in_A = cvo_align.number_of_non_zeros_in_A();
    std::cout << "The gpu inner product between " << i - 1 << " and " << i
              << " is " << in_product << "\n";
    // std::cout<<"The normalized inner product between "<<i-1 <<" and "<< i <<"
    // is "<<in_product_normalized<<"\n";
    ...
    // accum_output << accum_mat(0,0)<<" "<<accum_mat(0,1)<<"
    // "<<accum_mat(0,2)<<" "<<accum_mat(0,3)<<" "
    //             <<accum_mat(1,0)<<" " <<accum_mat(1,1)<<"
    //             "<<accum_mat(1,2)<<" "<<accum_mat(1,3)<<" "
    //             <<accum_mat(2,0)<<" " <<accum_mat(2,1)<<"
    //             "<<accum_mat(2,2)<<" "<<accum_mat(2,3);
    // accum_output<<"\n";
    // accum_output<<std::flush;

    Eigen::Quaternionf q(accum_mat.block<3, 3>(0, 0));
    accum_output << vstrRGBName[i + 1] << " ";
    accum_output << accum_mat(0, 3) << " " << accum_mat(1, 3) << " "
                 << accum_mat(2, 3) << " ";

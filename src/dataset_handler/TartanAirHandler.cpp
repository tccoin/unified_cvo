...
int TartanAirHandler::read_next_rgbd(cv::Mat& rgb_img, cv::Mat& dep_img) {
  if (curr_index >= total_size) return -1;
  // format curr_index
  stringstream ss;
  ...
  // read depth npy
  string dep_pth = folder_name + "/" + this->depth_folder_name + "/" +
                   index_str + "_left_depth.npy";
...
int TartanAirHandler::read_next_rgbd(cv::Mat& rgb_img,
...
int TartanAirHandler::read_next_rgbd(cv::Mat& rgb_img,
  ...
  if (read_next_rgbd(rgb_img, dep_vec)) return -1;
...
void TartanAirHandler::next_frame_index() { curr_index++; }

void TartanAirHandler::set_depth_folder_name(const std::string& folder) {
  this->depth_folder_name = folder;
}

void TartanAirHandler::set_start_index(int start) { curr_index = start; }

int TartanAirHandler::get_current_index() { return curr_index; }

int TartanAirHandler::get_total_number() { return total_size; }

int TartanAirHandler::read_next_semantics(int num_pixels,
                                          int num_semantic_class,
                                          std::vector<float>& semantics) {
  ...
  string semantic_name =
      folder_name + "/seg_left/" + index_str + "_left_seg.npy";
  ...
  for (int i = 0; i < num_pixels; i++) {
    ...
    semantics[begin_idx + remapped_label] =
        1.0;  // mark groundtruth with prob 1.0
  }
  return 0;
}

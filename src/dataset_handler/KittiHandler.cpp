...
int KittiHandler::read_next_stereo(cv::Mat &left, cv::Mat &right) {
  std::stringstream ss;
  ss << std::setw(6) << std::setfill('0') << curr_index;
  std::string file_name = ss.str();

  string left_name = folder_name + "/image_2/" + file_name + ".png";
  string right_name = folder_name + "/image_3/" + file_name + ".png";
  std::cout << "Reading stereo, left = " << left_name << std::endl;
  ...
  if (left.data == nullptr || right.data == nullptr) {
    cerr << "Image doesn't read successfully: " << left_name << ", "
         << right_name << "\n";
    return -1;
  }
  return 0;
...
int KittiHandler::read_next_stereo(cv::Mat &left, cv::Mat &right,
  ...
  if (read_next_stereo(left, right)) return -1;

  std::stringstream ss;
  ss << std::setw(6) << std::setfill('0') << curr_index;
  std::string file_name = ss.str();

  string semantic_name = folder_name + "/image_semantic/" + file_name + ".bin";
  ...
  if (debug_plot)
    visualize_semantic_image("last_semantic.png", semantics.data(),
                             num_semantic_class, left.cols, left.rows);
...
int KittiHandler::read_next_lidar(pcl::PointCloud<pcl::PointXYZI>::Ptr pc) {
  if (curr_index >= names.size()) return -1;

  string lidar_bin_path =
      folder_name + "/velodyne/" + names[curr_index] + ".bin";
  ...
  if (fLidar.is_open()) {
    ...
    Eigen::Affine3f ry = Eigen::Affine3f(
        Eigen::AngleAxisf(-M_PI / 2.0, Eigen::Vector3f::UnitY()));
    Eigen::Affine3f rz = Eigen::Affine3f(
...
int KittiHandler::read_next_lidar(pcl::PointCloud<pcl::PointXYZI>::Ptr pc,
  ...
  if (read_next_lidar(pc)) return -1;

  string semantic_bin_path =
      folder_name + "/labels/" + names[curr_index] + ".label";
  int num_semantic_class = 19;
  std::ifstream fLidar_sem(semantic_bin_path.c_str(),
  ...
  if (fLidar_sem.is_open()) {
    ...
    infile.open(semantic_bin_path.c_str(),
    ...
    for (int i = 0; i < num_lidar_points; i++) {
      ...
      uint16_t semantic_label =
          (uint16_t)(temp_semantics.data()[i] & 0x0000FFFFuL);
...
void KittiHandler::next_frame_index() { curr_index++; }

void KittiHandler::set_start_index(int start) { curr_index = start; }

int KittiHandler::get_current_index() { return curr_index; }

int KittiHandler::get_total_number() { return names.size(); }

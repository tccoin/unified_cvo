...
extern template struct pcl::PointSegmentedDistribution<FEATURE_DIMENSIONS,
                                                       NUM_CLASSES>;
...
CvoGPU::CvoGPU(const std::string& param_file) {
  ...
  cudaMemcpy((void*)params_gpu, &params, sizeof(CvoParams),
             cudaMemcpyHostToDevice);
}

void CvoGPU::write_params(const CvoParams* p_cpu) {
  // params = *p_cpu;
  cudaMemcpy((void*)params_gpu, p_cpu, sizeof(CvoParams),
             cudaMemcpyHostToDevice);
}

CvoGPU::~CvoGPU() { cudaFree(params_gpu); }

__device__ float compute_range_ell(float curr_ell, float curr_dist_to_sensor,
...
void update_tf(const Mat33f& R, const Vec3f& T,
               // outputs
               CvoState* cvo_state, Eigen::Ref<Mat44f> transform) {
  ...
  cudaMemcpy(cvo_state->R_gpu->data(), R_inv.data(), sizeof(Eigen::Matrix3f),
             cudaMemcpyHostToDevice);
  cudaMemcpy(cvo_state->T_gpu->data(), T_inv.data(), sizeof(Eigen::Vector3f),
             cudaMemcpyHostToDevice);
  // if (debug_print) std::cout<<"R,T is "<<R<<std::endl<<T<<std::endl;
  // if (debug_print) std::cout<<"transform mat
  // R"<<transform.block<3,3>(0,0)<<"\nT:
  // "<<transform.block<3,1>(0,3)<<std::endl;
}
...
void update_tf(const Mat33f & R, const Vec3f & T,
  ...
  cudaMemcpy(cvo_state->R_gpu->data(), R_inv.data(), sizeof(Eigen::Matrix3f),
cudaMemcpyHostToDevice); cudaMemcpy(cvo_state->T_gpu->data(), T_inv.data(),
...
__device__ Eigen::Matrix3f rotate_kernel_and_inverse(
    const CvoPoint& p_center, const Eigen::Matrix3f& eig) {
...
__device__ float mahananobis_distance(const CvoPoint& p_a, const CvoPoint& p_b,
                                      const Eigen::Matrix3f& kernel_inv) {
  // const Eigen::Matrix3f cov_a = Eigen::Map<const
  // Eigen::Matrix3f>(p_a.covariance); const Eigen::Matrix3f cov_b =
  // Eigen::Map<const Eigen::Matrix3f>(p_b.covariance); Eigen::Matrix3f cov =
  // cov_a + cov_b;
...
__device__ float eigenvalue_distance(const CvoPoint& p_a, const CvoPoint& p_b,
                                     float ell_shrink_ratio) {
  // const Eigen::Matrix3f cov_a = Eigen::Map<const
  // Eigen::Matrix3f>(p_a.covariance); const Eigen::Matrix3f cov_b =
  // Eigen::Map<const Eigen::Matrix3f>(p_b.covariance);
...
__device__ float compute_geometric_type_ip(const float* geo_type_a,
                                           const float* geo_type_b, int size) {
...
__global__ void fill_in_A_mat_gpu_dense_mat_kernel(  // input
    const CvoParams* cvo_params, CvoPoint* points_a, int a_size,
    CvoPoint* points_b, int b_size, int num_neighbors, Eigen::Matrix3f* kernel,
    // output
    SparseKernelMat* A_mat  // the inner product matrix!
) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i > a_size - 1) return;
  ...
  for (int j = 0; j < b_size; j++) {
    ...
    //   printf("i==%d, ell is %f, mahananobis_distance is %f, while d2_l2 is
    //   %f\n", i,ell_curr, d2, d2_l2);
    // }

    // float d2_iso =  squared_dist(*p_a, *p_b) / ell_curr / ell_curr;
    // float d2_iso =  squared_dist(*p_a, *p_b) / cvo_params->ell_init /
    // cvo_params->ell_init; d2 = d2 < d2_iso ? d2 : d2_iso;
    ...
    if (cvo_params->is_using_geometric_type) {
      geo_sim = compute_geometric_type_ip(p_a->geometric_type,
                                          p_b->geometric_type, 2);
      if (geo_sim < 0.01) continue;
    }

    if (cvo_params->is_using_geometry) {
      // if (i == 0)
      //   printf("computing mahanobis_dist");
      Eigen::Matrix3f rotated_kernel_inv =
          rotate_kernel_and_inverse(*p_a, *kernel);
    ...
    if (cvo_params->is_using_intensity) {
      float d2_color =
          squared_dist<float>(p_a->features, p_b->features, FEATURE_DIMENSIONS);
    ...
    if (cvo_params->is_using_semantics) {
      float d2_semantic = squared_dist<float>(
          p_a->label_distribution, p_b->label_distribution, NUM_CLASSES);
...
__global__ void fill_in_A_mat_cukdtree(const CvoParams* cvo_params,
  ...
  float a_to_sensor =
      sqrtf(p_a->x * p_a->x + p_a->y * p_a->y + p_a->z * p_a->z);
  ...
  if (cvo_params->is_using_semantics) label_a = p_a->label_distribution;
  ...
  for (int j = 0; j < num_neighbors; j++) {
    ...
    a = ck * k * sk * geo_sim;
    // if (i==0)
    //    printf("point_a is (%f,%f,%f), point_b is (%f,%f,%f), k=%f,ck=%f,
...
void find_nearby_source_points_cukdtree(  // const CvoParams *cvo_params,
    ...
    const Eigen::Matrix4f& transform_cpu_tf2sf, int num_neighbors,
    // output
  ...
  cudaMemcpy(R_gpu_t2s, &R_cpu_t2s, sizeof(decltype(R_cpu_t2s)),
             cudaMemcpyHostToDevice);
  cudaMemcpy(T_gpu_t2s, &T_cpu_t2s, sizeof(decltype(T_cpu_t2s)),
             cudaMemcpyHostToDevice);

  transform_pointcloud_thrust(cloud_x_gpu, cloud_x_gpu_transformed_kdtree,
                              R_gpu_t2s, T_gpu_t2s, false);

  kdtree.NearestKSearch(cloud_x_gpu_transformed_kdtree, num_neighbors,
                        cukdtree_inds_results);
  ...
  if (debug_print)
    std::cout << "Kdtree query time is " << totalTime << std::endl;
}

__global__ void fill_in_A_mat_gpu(const CvoParams* cvo_params,
                                  const CvoPoint* points_a, int a_size,
                                  const CvoPoint* points_b, int b_size,
                                  int num_neighbors, float ell,
                                  // output
  ...
  for (int j = 0; j < b_size; j++) {
    ...
    /*
    if (i == 0) {
      printf(" pointcloud_b.size is %d, num_neighbors is %d,  p_a[%d] is (%f,
    %f, %f), p_b[%d] is (%f, %f, %f\n)",    b_size, num_neighbors, i,
    p_a->x,p_a->y, p_a->z, ind_b, p_b->x, p_b->y, p_b->z);
             }*/
    float a = 1, sk = 1, ck = 1, k = 1, geo_sim = 1;
    if (cvo_params->is_using_geometric_type) {
      geo_sim = compute_geometric_type_ip(p_a->geometric_type,
    ...
    if (cvo_params->is_using_semantics) {
      float d2_semantic = squared_dist<float>(
          p_a->label_distribution, p_b->label_distribution, NUM_CLASSES);
      if (d2_semantic < d2_s_thres)
        sk = cvo_params->s_sigma * cvo_params->s_sigma *
             exp(-d2_semantic / (2.0 * s_ell * s_ell));
      else
        continue;
    }
    a = ck * k * sk * geo_sim;
    /*
    if (i==0)  {
      printf("point_a is (%f,%f,%f), point_b with index %d is (%f,%f,%f),
    k=%f,ck=%f, sk=%f \n", p_a->x, p_a->y, p_a->z, ind_b,  p_b->x, p_b->y,
    p_b->z,  k, ck, sk );
      }*/
...
static void se_kernel_kdtree(  // input
    const CvoParams& params_cpu, const CvoParams* params_gpu,
    std::shared_ptr<CvoPointCloudGPU> points_fixed,
    std::shared_ptr<CvoPointCloudGPU> points_moving, float ell,
    perl_registration::cuKdTree<CvoPoint>& kdtree,
    const Eigen::Matrix4f& transform_cpu_tf2sf, int num_neighbors,
    // output
    std::shared_ptr<CvoPointCloudGPU> cloud_x_gpu_transformed_kdtree,
    thrust::device_vector<int>& cukdtree_inds_results, SparseKernelMat* A_mat,
    SparseKernelMat* A_mat_gpu) {
  find_nearby_source_points_cukdtree(
      points_fixed, kdtree, transform_cpu_tf2sf, num_neighbors,
      cloud_x_gpu_transformed_kdtree, cukdtree_inds_results);

  /*
  std::cout<<"1st point neighbors are:\n";
  int inds[64];
  cudaMemcpy(inds, thrust::raw_pointer_cast(cukdtree_inds_results.data()),
  sizeof(int)*64, cudaMemcpyDeviceToHost); std::cout<<"neighbors of pt_fixed[0]:
  "; for (int i = 0; i < 32; i++) { std::cout<<inds[i]<<" ";
  }
  ...
  */
  ...
  fill_in_A_mat_cukdtree<<<(points_fixed->points.size() / CUDA_BLOCK_SIZE) + 1,
                           CUDA_BLOCK_SIZE>>>(
      params_gpu, thrust::raw_pointer_cast(points_fixed->points.data()),
      points_fixed->size(),
      thrust::raw_pointer_cast(points_moving->points.data()),
      points_moving->size(),
      thrust::raw_pointer_cast(cukdtree_inds_results.data()), num_neighbors,
      ell, A_mat_gpu);
...
static void se_kernel(  // input
    const CvoParams* params_gpu, std::shared_ptr<CvoPointCloudGPU> points_fixed,
    std::shared_ptr<CvoPointCloudGPU> points_moving, int num_neighbors,
    float ell,
    // output
    SparseKernelMat* A_mat, SparseKernelMat* A_mat_gpu) {
  auto start = chrono::system_clock::now();
  int fixed_size = points_fixed->points.size();
  CvoPoint* points_fixed_raw =
      thrust::raw_pointer_cast(points_fixed->points.data());
  CvoPoint* points_moving_raw =
      thrust::raw_pointer_cast(points_moving->points.data());

  fill_in_A_mat_gpu<<<(points_fixed->points.size() / CUDA_BLOCK_SIZE) + 1,
                      CUDA_BLOCK_SIZE>>>(
      params_gpu, points_fixed_raw, fixed_size, points_moving_raw,
      points_moving->points.size(), num_neighbors, ell,
      // output
...
static void se_kernel_dense(  // input
    const CvoParams* params_gpu, std::shared_ptr<CvoPointCloudGPU> points_fixed,
    std::shared_ptr<CvoPointCloudGPU> points_moving, int num_neighbors,
  ...
  cudaMemcpy(kernel_gpu, &kernel_cpu, sizeof(decltype(kernel_cpu)),
             cudaMemcpyHostToDevice);
  // std::cout<<"Start fill_in_A_mat_gpu_dense_mat_kernel\n"<<std::endl;
  fill_in_A_mat_gpu_dense_mat_kernel<<<
      (points_fixed->points.size() / CUDA_BLOCK_SIZE) + 1, CUDA_BLOCK_SIZE>>>(
      params_gpu, points_fixed_raw, fixed_size, points_moving_raw,
      points_moving->points.size(), num_neighbors, kernel_gpu,
      // output
      A_mat_gpu  // the kernel mat
  );
  cudaDeviceSynchronize();
...
void compute_flow(CvoState* cvo_state, const CvoParams* params_gpu,
  ...
  if (debug_print) {
    ...
    std::cout << "A rows is " << cvo_state->A_host.rows << ", A cols is "
              << cvo_state->A_host.cols << std::endl;
  }
  auto start = chrono::system_clock::now();
  compute_flow_gpu_no_eigen<<<cvo_state->A_host.rows / CUDA_BLOCK_SIZE + 1,
                              CUDA_BLOCK_SIZE>>>(
      params_gpu,
      thrust::raw_pointer_cast(cvo_state->cloud_x_gpu->points.data()),
      thrust::raw_pointer_cast(cvo_state->cloud_y_gpu->points.data()),
      cvo_state->A, num_neighbors,
  ...
  // std::cout<<"time for compute_gradient is
  ...
  //(thrust::reduce(thrust::device, cvo_state->omega_gpu.begin(),
  // cvo_state->omega_gpu.end(), Eigen::Vector3d(0,0,0)  ) );
  *omega =
      (thrust::reduce(thrust::device, cvo_state->omega_gpu.begin(),
                      cvo_state->omega_gpu.end(), Eigen::Vector3d(0, 0, 0)))
          .cast<float>();
  *v = (thrust::reduce(thrust::device, cvo_state->v_gpu.begin(),
                       cvo_state->v_gpu.end(), Eigen::Vector3d(0, 0, 0)))
           .cast<float>();
  ...
  *v = ov.segment<3>(3);
  ...
  cudaMemcpy(cvo_state->omega, omega, sizeof(Eigen::Vector3f),
             cudaMemcpyHostToDevice);
  cudaMemcpy(cvo_state->v, v, sizeof(Eigen::Vector3f), cudaMemcpyHostToDevice);

  end = chrono::system_clock::now();
  // std::cout<<"time for thrust_reduce is
  // "<<std::chrono::duration_cast<std::chrono::milliseconds>((end-
  // start)).count()<<std::endl;
  start = chrono::system_clock::now();
  int A_nonzero = nonzeros(&cvo_state->A_host);
  if (debug_print)
    std::cout << "compute flow result: omega " << omega->transpose()
              << ", v: " << v->transpose() << std::endl;
  end = chrono::system_clock::now();
  // std::cout<<"time for nonzeros
  // "<<std::chrono::duration_cast<std::chrono::milliseconds>((end-
  // start)).count()<<std::endl;
}

__global__ void fill_in_residual_and_jacobian(
    float ell, CvoPoint* cloud_x, CvoPoint* cloud_y, SparseKernelMat* A,
    // output
    Eigen::Matrix<float, 6, 6>* ls_lhs, Eigen::Matrix<float, 6, 1>* ls_rhs) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i > A->rows - 1) return;
  ...
  for (int j = 0; j < A->cols; j++) {
    ...
    float dist =
        sqrtf(r_ij(0) * r_ij(0) + r_ij(1) * r_ij(1) + r_ij(2) * r_ij(2)) * ell;
    if (dist > 0.2) return;
  ...
  if (i < 100) {
    // printf("fill-in-least-square: i=%d, j=%d, x=(%f,%f,%f), y=(%f,%f,%f),
    // |x-y| is %f, J_i(3,3) is \n%f, %f, %f\n %f, %f, %f\n %f,%f,%f\n",
    // i,x(0),x(1),x(2), y(0),y(1),y(2),  dist , J_i(0,0), J_i(0,1), J_i(0,2),
    // J_i(1,0), J_i(1,1), J_i(1,2), J_i(2,0), J_i(2,1), J_i(2,2)  );
  }
}

void compute_flow_least_square(CvoState* cvo_state, const CvoParams* params_gpu,
                               Eigen::Vector3f* omega, Eigen::Vector3f* v) {
  fill_in_residual_and_jacobian<<<cvo_state->A_host.rows / CUDA_BLOCK_SIZE + 1,
                                  CUDA_BLOCK_SIZE>>>(
      cvo_state->ell,
      thrust::raw_pointer_cast(cvo_state->cloud_x_gpu->points.data()),
      thrust::raw_pointer_cast(cvo_state->cloud_y_gpu->points.data()),
  ...
  auto ls_lhs = thrust::reduce(cvo_state->least_square_LHS.begin(),
                               cvo_state->least_square_LHS.end(), zero_66);

  Eigen::Matrix<float, 6, 1> zero_61;
  zero_61 = Eigen::Matrix<float, 6, 1>::Zero().eval();
  Eigen::Matrix<float, 6, 1> ls_rhs =
      thrust::reduce(cvo_state->least_square_RHS.begin(),
                     cvo_state->least_square_RHS.end(), zero_61);
...
__global__ void compute_step_size_xi(Eigen::Vector3f* omega, Eigen::Vector3f* v,
                                     CvoPoint* cloud_y, int num_moving,
                                     int num_neighbors,
                                     // outputs
                                     Eigen::Vector3f_row* xiz,
                                     Eigen::Vector3f_row* xi2z,
                                     Eigen::Vector3f_row* xi3z,
                                     Eigen::Vector3f_row* xi4z, float* normxiz2,
                                     float* xiz_dot_xi2z, float* epsil_const) {
  int j = blockIdx.x * blockDim.x + threadIdx.x;
  if (j > num_moving - 1) return;
  ...
  xi2z[j] = (omega_hat * omega_hat * cloud_yi + (omega_hat * (*v)))
                .transpose();  // (xi^2*z+xi*v)
  xi3z[j] = (omega_hat * omega_hat * omega_hat * cloud_yi +
             (omega_hat * omega_hat * (*v)))
                .transpose();  // (xi^3*z+xi^2*v)
  xi4z[j] = (omega_hat * omega_hat * omega_hat * omega_hat * cloud_yi +
             (omega_hat * omega_hat * omega_hat * (*v)))
                .transpose();  // (xi^4*z+xi^3*v)
  normxiz2[j] = xiz[j].squaredNorm();
  xiz_dot_xi2z[j] = (-xiz[j].dot(xi2z[j]));
  epsil_const[j] = xi2z[j].squaredNorm() + 2 * xiz[j].dot(xi3z[j]);
  /*
    if ( j == 1000) {
    printf("j==1000, cloud+yi is (%f,%f,%f), xiz=(%f %f %f), xi2z=(%f %f %f),
    xi3z=(%f %f %f), xi4z=(%f %f %f), normxiz2=%f, xiz_dot_xi2z=%f,
    epsil_const=%f\n ", cloud_y[j].x , cloud_y[j].y, cloud_y[j].z, xiz[j](0),
    xiz[j](1), xiz[j](2), xi2z[j](0),xi2z[j](1),xi2z[j](2),
...
__global__ void compute_step_size_poly_coeff(
    float ell, float ell_init, int is_using_location_dependent_ell,
    int num_moving, SparseKernelMat* A, CvoPoint* cloud_x, CvoPoint* cloud_y,
    Eigen::Vector3f_row* xiz, Eigen::Vector3f_row* xi2z,
    Eigen::Vector3f_row* xi3z, Eigen::Vector3f_row* xi4z, float* normxiz2,
    float* xiz_dot_xi2z, float* epsil_const, int num_neighbors,
    // output
  ...
  for (int j = 0; j < num_neighbors; j++) {
    ...
    /*
      #ifdef IS_USING_COVARIANCE
      temp_ell = (cloud_x[i].cov_eigenvalues[2] +
      cloud_y[idx].cov_eigenvalues[2] + cloud_x[i].cov_eigenvalues[0] +
      cloud_y[idx].cov_eigenvalues[0])/4.0 ;
      //temp_ell = ( cloud_x[i].cov_eigenvalues[2] +
      cloud_y[idx].cov_eigenvalues[2])/2.0 ; temp_ell *= (ell/ell_init); if
      (temp_ell > 1.0) temp_ell = 1.0; if (temp_ell < 0.01) temp_ell =0.01;
      #endif  */
    ...
    float gamma_ij =
        (-temp_coef * (normxiz2[idx] + (2.0 * xi2z[idx] * diff_xy).value()));
    // delta_i = 1/l^2 * (dot(-xiz,xi2z) + dot(-xi3z,diff_xy))
    float delta_ij = (2.0 * temp_coef *
                      (xiz_dot_xi2z[idx] + (-xi3z[idx] * diff_xy).value()));
    // epsil_i = -1/(2*l^2) * (norm(xi2z).^2 + 2*dot(xiz,xi3z) +
    // 2*dot(xi4z,diff_xy))
    float epsil_ij =
        (-temp_coef * (epsil_const[idx] + (2.0 * xi4z[idx] * diff_xy).value()));
    ...
    C[i] += ci;
    double di = double(A_ij * (delta_ij + beta_ij * gamma_ij +
                               beta_ij * beta_ij * beta_ij / 6.0));
    D[i] += di;
    double ei =
        double(A_ij * (epsil_ij + beta_ij * delta_ij +
                       1 / 2.0 * beta_ij * beta_ij * gamma_ij +
                       1 / 2.0 * gamma_ij * gamma_ij +
                       1 / 24.0 * beta_ij * beta_ij * beta_ij * beta_ij));
    E[i] += ei;
  }
}
...
void compute_step_size(CvoState* cvo_state, const CvoParams* params,
                       int num_neighbors) {
  compute_step_size_xi<<<cvo_state->num_moving / CUDA_BLOCK_SIZE + 1,
                         CUDA_BLOCK_SIZE>>>(
      cvo_state->omega, cvo_state->v,
      thrust::raw_pointer_cast(cvo_state->cloud_y_gpu->points.data()),
      cvo_state->num_moving, num_neighbors,
  ...
  // compute_step_size_poly_coeff_range_ell<<<cvo_state->num_fixed /
  // CUDA_BLOCK_SIZE + 1, CUDA_BLOCK_SIZE>>>
  compute_step_size_poly_coeff<<<cvo_state->num_fixed / CUDA_BLOCK_SIZE + 1,
                                 CUDA_BLOCK_SIZE>>>(
      ...
      thrust::raw_pointer_cast(cvo_state->normxiz2.data()),
      thrust::raw_pointer_cast(cvo_state->xiz_dot_xi2z.data()),
      thrust::raw_pointer_cast(cvo_state->epsil_const.data()), num_neighbors,
  ...
  thrust::plus<double> plus_double;
  double B = thrust::reduce(cvo_state->B.begin(), cvo_state->B.end(), 0.0,
                            plus_double);
  double C = thrust::reduce(cvo_state->C.begin(), cvo_state->C.end(), 0.0,
                            plus_double);
  double D = thrust::reduce(cvo_state->D.begin(), cvo_state->D.end(), 0.0,
                            plus_double);
  double E = thrust::reduce(cvo_state->E.begin(), cvo_state->E.end(), 0.0,
                            plus_double);
  ...
  if (debug_print) std::cout << "BCDE is " << p_coef.transpose() << std::endl;
  ...
  for (int i = 0; i < rc.real().size(); i++) {
    if (rc(i, 0).real() > 0 && rc(i, 0).real() < temp_step &&
        std::fabs(rc(i, 0).imag()) < 1e-5) {
  ...
  cvo_state->step =
      temp_step == numeric_limits<double>::max() ? params->min_step : temp_step;
  ...
  if (debug_print) std::cout << "step size " << cvo_state->step << "\n";
}

static bool A_sparsity_indicator_ell_update(
    std::queue<float>& indicator_start_queue,
    std::queue<float>& indicator_end_queue, float& indicator_start_sum,
    float& indicator_end_sum, const float indicator, const CvoParams& params) {
  ...
  if (indicator_start_queue.size() >= queue_len &&
      indicator_end_queue.size() < queue_len) {
  ...
  // static std::ofstream ratio("ip_ratio.txt",std::ofstream::out |
  // std::ofstream::app );
  //  start queue is full, start building the end queue
  if (indicator_start_queue.size() >= queue_len &&
      indicator_end_queue.size() >= queue_len) {
    // ratio<< indicator_end_sum / indicator_start_sum<<"\n"<<std::flush;
    // std::cout<<"ip ratio is "<<indicator_end_sum / indicator_start_sum<<"
    // from "<<indicator_end_sum<<" over "<<indicator_start_sum<<std::endl;
    //  check if criteria for decreasing legnthscale is satisfied
    if (indicator_end_sum / indicator_start_sum >
            1 - params.indicator_stable_threshold &&
        indicator_end_sum / indicator_start_sum <
            1 + params.indicator_stable_threshold) {
      decrease = true;
  ...
  // DEBUG
  // std::cout << "indicator=" << indicator << ", start size=" <<
  // indicator_start_queue.size() << ", sum=" << indicator_start_sum \
    // << ", end size=" << indicator_end_queue.size() << ", sum=" <<
  // indicator_end_sum << std::endl;
...
__global__ void fill_in_A_mat_euclidean(  // input
    CvoPoint* points_a, int a_size, CvoPoint* points_b, int b_size,
    // output
    double* l2_sum) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i > a_size - 1) return;
...
float compute_std_dist_between_two_pc(
    std::shared_ptr<CvoPointCloudGPU> source_points,
  ...
  CvoPoint* points_fixed_raw =
      thrust::raw_pointer_cast(source_points->points.data());
  CvoPoint* points_moving_raw =
      thrust::raw_pointer_cast(target_points_transformed->points.data());

  fill_in_A_mat_euclidean<<<(num_points_target / CUDA_BLOCK_SIZE) + 1,
                            CUDA_BLOCK_SIZE>>>(
      points_fixed_raw, num_points_source, points_moving_raw, num_points_target,
      thrust::raw_pointer_cast(l2_sum.data()));

  double dist_square_sum = thrust::reduce(l2_sum.begin(), l2_sum.end());
  double std_dist = sqrt(dist_square_sum / (double)num_points_source /
                         (double)num_points_target);

  return static_cast<float>(std_dist);
}

static __attribute__((force_align_arg_pointer)) int align_impl(
    const CvoParams& params, const CvoParams* params_gpu, CvoState& cvo_state,
    const Eigen::Matrix4f& init_guess_transform,
    // output
    Eigen::Ref<Eigen::Matrix4f> transform, Association* association_mat,
  ...
  int k = 0;
  int num_neighbors = params.is_using_kdtree ? perl_registration::KDTREE_K_SIZE
                                             : params.nearest_neighbors_max;
  Eigen::Matrix<double, 4, 4, Eigen::DontAlign> dRT =
      Eigen::Matrix<double, 4, 4, Eigen::DontAlign>::Identity();
  for (; k < params.MAX_ITER; k++) {
    ...
    if (is_logging) {
      ...
      transform_file << Tmat(0, 0) << " " << Tmat(0, 1) << " " << Tmat(0, 2)
                     << " " << Tmat(0, 3) << " " << Tmat(1, 0) << " "
                     << Tmat(1, 1) << " " << Tmat(1, 2) << " " << Tmat(1, 3)
                     << " " << Tmat(2, 0) << " " << Tmat(2, 1) << " "
                     << Tmat(2, 2) << " " << Tmat(2, 3) << "\n"
                     << std::flush;
    }
    ...
    transform_pointcloud_thrust(cvo_state.cloud_y_gpu_init,
                                cvo_state.cloud_y_gpu, cvo_state.R_gpu,
                                cvo_state.T_gpu, false);

    // update the inner product matrix
    if (params.is_using_kdtree) {
      se_kernel_kdtree(
          params, params_gpu, cvo_state.cloud_x_gpu, cvo_state.cloud_y_gpu,
          cvo_state.ell, *cvo_state.kdtree_moving_points, transform,
          num_neighbors, cvo_state.cloud_x_gpu_transformed_kdtree,
          cvo_state.kdtree_inds_results, &cvo_state.A_host, cvo_state.A);
    } else
      se_kernel(params_gpu, cvo_state.cloud_x_gpu, cvo_state.cloud_y_gpu,
                num_neighbors, cvo_state.ell, &cvo_state.A_host, cvo_state.A);
    cudaDeviceSynchronize();
    if (debug_print) {
      float sum_A = thrust::reduce(
          thrust::device, cvo_state.A_host.mat,
          cvo_state.A_host.mat + cvo_state.A_host.rows * num_neighbors,
          (float)0);
      std::cout << "nonzeros in A " << nonzeros(&cvo_state.A_host)
                << ", sum of A is " << sum_A << std::endl;
      ...
      std::cout << "A rows is " << cvo_state.A_host.rows << ", A cols is "
                << cvo_state.A_host.cols << std::endl;
      if (k % 100 == 0) {
        std::ofstream neighbors_dist(
            "neighbors_dist_" + std::to_string(k) + ".txt", std::ofstream::out);
        thrust::host_vector<int> neighbors(cvo_state.A_host.rows);
        cudaMemcpy(neighbors.data(), cvo_state.A_host.nonzeros,
                   neighbors.size() * sizeof(unsigned int),
                   cudaMemcpyDeviceToHost);
        for (auto p : neighbors) neighbors_dist << p << "\n";
    ...
    // float indicator = (double)cvo_state.A_host.nonzero_sum /
    // (double)source_points.num_points() / (double) target_points.num_points();
    ...
    cudaMemcpy(omega.data(), cvo_state.omega->data(), sizeof(Eigen::Vector3f),
               cudaMemcpyDeviceToHost);
    cudaMemcpy(v.data(), cvo_state.v->data(), sizeof(Eigen::Vector3f),
               cudaMemcpyDeviceToHost);
    if (omega.cast<double>().norm() < params.eps &&
        v.cast<double>().norm() < params.eps) {
      std::cout << "break: norm, omega: " << omega.norm() << ", v: " << v.norm()
                << std::endl;
    ...
    vec_joined << omega, v;
    Eigen::Matrix<float, 3, 4> dtrans =
        Exp_SEK3(vec_joined, cvo_state.step).cast<float>();
    ...
    float ip_curr = (float)((double)cvo_state.A_host.nonzero_sum /
                            sqrt((double)cvo_state.num_fixed *
                                 (double)cvo_state.num_moving));
    // float ip_curr = (float) this->inner_product(source_points, target_points,
    // transform);
    bool need_decay_ell = A_sparsity_indicator_ell_update(
        indicator_start_queue, indicator_end_queue, indicator_start_sum,
        indicator_end_sum, ip_curr, params);

    if (is_logging) {
      ell_file << cvo_state.ell << "\n" << std::flush;
      dist_change_file << dist_this_iter << "\n" << std::flush;

      // float ip_curr = (double)cvo_state.A_host.nonzero_sum /
      // (double)source_points.num_points() / (double)
      // target_points.num_points(); effective_points_file << indicator << "\n"
      // << std::flush;
      inner_product_file << ip_curr << "\n" << std::flush;
      // inner_product_file<<this->inner_product(source_points, target_points,
      // transform)<<"\n"<<std::flush;
    }
    if (debug_print)
      std::cout << "dist: " << dist_this_iter << std::endl
    ...
    if (k > params.ell_decay_start && need_decay_ell) {
      ...
      if (cvo_state.ell < params.ell_min) cvo_state.ell = params.ell_min;
    }
    if (params.is_using_kdtree)
      num_neighbors =
          std::min(perl_registration::KDTREE_K_SIZE,
                   (int)std::ceil((double)nonzeros(&cvo_state.A_host) /
                                  (double)cvo_state.A_host.rows) *
                       params.is_using_kdtree);
    else {
      thrust::device_ptr<unsigned int> max_ind =
          thrust::device_pointer_cast(cvo_state.A_host.nonzeros);
      unsigned int max_ind_val = *thrust::max_element(
          thrust::device, max_ind, max_ind + cvo_state.A_host.rows);
      if (debug_print) {
        std::cout << "max number of neighbors is " << max_ind_val << std::endl;
      }
      if (is_logging) {
        std::ofstream max_neighbor_f("max_neibor_all.txt",
      ...
      num_neighbors =
          std::min(params.nearest_neighbors_max, (int)(max_ind_val * 1.2));
    }
    if (debug_print)
      std::cout << "num_neighbors is " << num_neighbors << std::endl;
  }
  cudaEventRecord(cuda_stop, 0);
  cudaEventSynchronize(cuda_stop);
  float elapsedTime, totalTime;
  ...
  std::cout << "non adaptive cvo ends. final ell is " << cvo_state.ell
            << ", final iteration is " << k << "MAX_ITER is " << params.MAX_ITER
            << std::endl;

  if (params.is_exporting_association && association_mat)
    gpu_association_to_cpu(cvo_state.A_host, *association_mat,
                           cvo_state.num_fixed, cvo_state.num_moving,
                           num_neighbors);
...
int CvoGPU::align(const pcl::PointCloud<CvoPoint>& source_points,
  ...
  std::cout << "construct new cvo state..., init ell is " << cvo_state.ell
            << std::endl;

  int ret = align_impl(params, params_gpu, cvo_state, init_guess_transform,
                       transform, association_mat, registration_seconds);
  // std::cout<<"Result Transform is "<<transform<<std::endl;
  return ret;
}

int CvoGPU::align(  // inputs
    const CvoPointCloud& source_points, const CvoPointCloud& target_points,
    const Eigen::Matrix4f& init_guess_transform,
    // outputs
    Eigen::Ref<Eigen::Matrix4f> transform, Association* association_mat,
    double* registration_seconds) const {
...
int CvoGPU::align(  // inputs
                    ...
                    // Eigen::aligned_allocator<Eigen::Matrix4f>> & poses_in,
  ...
  for (auto&& p : edges) {
    ...
    if (params.multiframe_using_cpu) {
      ...
      binary_states.push_back(
          std::dynamic_pointer_cast<BinaryState>(new_binary_state));
    } else {
      BinaryStateGPU::Ptr new_binary_state(new BinaryStateGPU(
          std::dynamic_pointer_cast<CvoFrameGPU>(f1),
          std::dynamic_pointer_cast<CvoFrameGPU>(f2), &params, params_gpu,
  ...
  CvoBatchIRLS batch_irls_problem(frames, frames_to_hold_const, binary_states,
                                  &params);
...
float CvoGPU::inner_product(const CvoPointCloud& source_points,
  ...
  A_mat.setZero();
  se_kernel_init_ell_cpu(&source_points, &target_points, &fixed_positions,
&moving_positions, A_mat, A_trip_concur_ , params );
  ...
  //return float(A_mat.nonZeros()) / float(fixed_positions.size()) /
float(moving_positions.size() ) * 100 ;
  //}
...
static
    ...
    float
    inner_product_impl(CvoPointCloudGPU::SharedPtr source_gpu,
                       CvoPointCloudGPU::SharedPtr target_gpu,
                       const Eigen::Matrix4f& init_guess_transform,
                       const CvoParams& params, const CvoParams* params_gpu,
                       bool is_using_isotropic_kernel, float curr_ell,
  ...
  if (is_using_isotropic_kernel)
    ...
    se_kernel_dense(  // input
        ...
        params.nearest_neighbors_max, *non_isotropic_kernel,
        // output
        &cvo_state.A_host, cvo_state.A);
  }
  cudaDeviceSynchronize();
...
float CvoGPU::inner_product_gpu(const CvoPointCloud& source_points,
  ...
  return inner_product_impl(source_gpu, target_gpu, init_guess_transform,
                            params, params_gpu, true, ell, nullptr, nullptr);
}

float CvoGPU::inner_product_gpu(
    const pcl::PointCloud<CvoPoint>& source_points_pcl,
    const pcl::PointCloud<CvoPoint>& target_points_pcl,
    const Eigen::Matrix4f& init_guess_transform, float ell) const {
  CvoPointCloudGPU::SharedPtr source_gpu =
      pcl_PointCloud_to_gpu(source_points_pcl);
  CvoPointCloudGPU::SharedPtr target_gpu =
      pcl_PointCloud_to_gpu(target_points_pcl);
...
float CvoGPU::function_angle(const CvoPointCloud& source_points,
  ...
  if (is_approximate) {
    ...
    if (is_gpu) {
      fx_norm =
          sqrt(inner_product_gpu(source_points, source_points, identity, ell));
      fz_norm =
          sqrt(inner_product_gpu(target_points, target_points, identity, ell));
    } else {
      fx_norm =
          sqrt(inner_product_cpu(source_points, source_points, identity, ell));
      fz_norm =
          sqrt(inner_product_cpu(target_points, target_points, identity, ell));
    }
  }
  cosine_value = fxfz / (fx_norm * fz_norm);
  std::cout << "fxfz is " << fxfz << ", fx_norm is " << fx_norm
            << ", fz_norm is " << fz_norm << std::endl;
  return cosine_value;
}
...
float CvoGPU::function_angle(const pcl::PointCloud<CvoPoint>& source_points_pcl,
                             const pcl::PointCloud<CvoPoint>& target_points_pcl,
                             const Eigen::Matrix4f& t2s_frame_transform,
                             float ell, bool is_approximate) const {
  ...
  Eigen::Matrix4f identity = Eigen::Matrix4f::Identity();
  float fxfz = 0, fx_norm = 0, fz_norm = 0, cosine_value = 0;
  fxfz = inner_product_gpu(source_points_pcl, target_points_pcl,
                           t2s_frame_transform, ell);
  if (is_approximate) {
    fx_norm = sqrt(source_points_pcl.size());
    fz_norm = sqrt(target_points_pcl.size());
  } else {
    fx_norm = sqrt(
        inner_product_gpu(source_points_pcl, source_points_pcl, identity, ell));
    fz_norm = sqrt(
        inner_product_gpu(target_points_pcl, target_points_pcl, identity, ell));
  }
  cosine_value = fxfz / (fx_norm * fz_norm);
...
void CvoGPU::compute_association_gpu(
    const CvoPointCloud& source_points, const CvoPointCloud& target_points,
    const Eigen::Matrix4f& T_target_frame_to_source_frame, float lengthscale,
    // output
  ...
  inner_product_impl(source_gpu, target_gpu, T_target_frame_to_source_frame,
                     params, params_gpu, true, lengthscale, nullptr,
                     &association);
...
static
    //  __attribute__((force_align_arg_pointer))
    void
    inner_product_non_isotropic_impl(
        CvoPointCloudGPU::SharedPtr source_gpu,
        CvoPointCloudGPU::SharedPtr target_gpu,
        const Eigen::Matrix4f& init_guess_transform, const CvoParams& params,
  ...
  cudaMemcpy((void*)new_params_gpu, &new_params, sizeof(CvoParams),
             cudaMemcpyHostToDevice);

  se_kernel_dense(  // input
      new_params_gpu, cvo_state.cloud_x_gpu, cvo_state.cloud_y_gpu,
      params.nearest_neighbors_max, kernel_inv,
      // output
      &cvo_state.A_host, cvo_state.A);
...
void CvoGPU::compute_association_gpu(
  ...
  // this->write_params(&params_tmp);
  inner_product_non_isotropic_impl(
      source_gpu, target_gpu, T_target_frame_to_source_frame, params,
      params_gpu, &non_isotropic_kernel, &association);
}

int CvoGPU::align(  // inputs
    std::vector<CvoFrame::Ptr>&
        frames,  // point clouds, poses, the outputs are within
    const std::vector<bool>& frames_to_hold_const,
    const std::list<BinaryState::Ptr>& edge_states,
    // outputs
    double* registration_seconds,
    std::list<std::shared_ptr<Association>>* inliers) const {
  std::cout << "CvoGPU.cpp:align() get called\n";

  auto start = std::chrono::system_clock::now();
  CvoBatchIRLS batch_irls_problem(frames, frames_to_hold_const, edge_states,
                                  &params);
  ...
  if (registration_seconds)
    *registration_seconds = (double)t_all.count() / 1000;
  if (inliers) {
    for (auto state_ptr : edge_states) {
      Association::Ptr assoc(new Association);
      state_ptr->export_association(*assoc);
      inliers->push_back(assoc);
    }
  }
  return 0;
}
